...
<script>
  ...
  import {
    getTemplateInfosByGid,
    getTemplatePropertiesByGid,
    ddlPreviewCode,
    ddlGenerateCode
  } from '@/api/template'
      ...
      generateCode(){
        ...
        this.form.validateFields((errors, values) => {
          if (!errors){
            ...
            queryParam.dcStatements=[this.model.code]
            console.log("表单内容",queryParam)
            ddlGenerateCode(this.model.dsName,queryParam).then(fileBlob=>{
      ...
      previewCode() {
        this.form.validateFields((err, values) => {
          if (!err) {
            ...
            let queryParam=this.form.getFieldsValue();
            queryParam.dcStatements=[this.model.code]
            ddlPreviewCode(this.model.dsName,queryParam).then(res=>{
